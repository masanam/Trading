    ...
    $scope.createDeal = function(){
      if($scope.buyOrders.length > 0 && $scope.sellOrders.length > 0){
        ...
        deal.$save(function (response) {
          ...
          var actionSuccess = function (response) {
          };
          var actionError = function (response) {
            $scope.error = response.data.message;
          };

          for(i = 0; i < $scope.sellOrders.length; i++){
            ...
            sellDeal.$save(actionSuccess, actionError);
          }
          
          for(i = 0; i < $scope.buyOrders.length; i++){
            ...
            buyDeal.$save(actionSuccess, actionError);
          }
...
angular.module('deal').controller('BuyDealChatModalController', function ($scope, $uibModalInstance, Pusher, User, BuyDealChat, buyDeal, Authentication) {
  ...
  $scope.findCurrentUser = function() {
    $scope.user = User.get({ action: Authentication.user });
  };
...
angular.module('deal').controller('SellDealChatModalController', function ($scope, $uibModalInstance, Pusher, User, SellDealChat, sellDeal, Authentication) {
